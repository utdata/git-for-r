...
> 🎯 Goal: By the end of this chapter, you'll know how to setup a Github project for collaboration. You'll also know some best practices when working with others on a shared project.
...
1.  Add the teammate’s GitHub username.
...
1.  If you need to **create** a folder for your projects, you can do this in the terminal like this:
...
1.  Hit `Enter` / `return` to run the command.
...
1.  Locate the name of your cloned repo in the files list.
1.  Use the `cd` commmand to navigate into your repo folder, replacing `folder_name` with your repo name:

    ``` bash
    the cd folder_name
    ```

1.  Now, run the `ls` command again to see your project files.
...
1.  Staging and committing the changes using Git (`git add`, `git commit -m`).
1.  Pushing those changes to GitHub (`git push`).
