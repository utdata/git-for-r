...
1.  **Install Git**
    ...
    1.  Default install settings are fine
...
1.  Check to see if you successfully set your name by running the command:
...
1.  Launch RStudio.
1.  Go to the menu Tools \> Global Options, then click on the **Git/SVN tab**.
1.  Check the **Enable version control interface for RStudio** button.
1.  There should already be a path included in the **git executable** field.
1.  Make sure the path to the git executable is correct.
...
1.  If you SSH Key path is already filled in, then you can skip to the **"Add SSH Keys on Github"** section. If you do not have an SSH key showing, follow the next steps below to make one before moving on.
...
1.  You should be able to leave all the defaults as they are. **Leave the optional Passphrase field blank.**
1.  Click the **Create** button.
1.  You should see a message that says "Key Created Successfully" and it might show you a pretty picture. Click **OK**.
...
1.  This will open a new window with the contents of your public key.
1.  Copy the full contents of the public key.
...
1.  In the "Title" field, add a descriptive label for the new key. Name it something like "Personal MacBook Air" to indicate its your personal computer.
1.  In the "Key" field, paste ( `Command`+`V)`the public key you copied from RStudio into the box.
