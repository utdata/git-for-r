...
2.  Click the **+** in toolbar at the upper right corner. Then click **"New
    repository"** in the dropdown.

3.  Do the following in the **Create a new repository** form:

    1.   Fill in the **Repository name** field, using the same name as
        your project folder (e.g. `my-quarto-project`)

    2.   Leave **Description** blank for now

    3.   Leave the box **unchecked** that says "Initialize this
...
5. In the **Quick setup** box, click the copy icon button next to the "SSH URL" box.
...
2. Type the following command, but replace the URL with the one you copied from GitHub:
    
    ``` bash
    git remote add origin git@github.com:your-name/your-repo-name.git
    ```
4. Run the command by hitting `Enter`/`return`.
...
## Quick Recap: what do these cmmands do?
...
-   `git push -u origin main`
    ...
    2. "Pushed" your local commits (aka the "snapshot" we took last section) to GitHub.
